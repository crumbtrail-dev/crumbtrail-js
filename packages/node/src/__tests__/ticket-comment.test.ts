...
const SESSION_URL_BASE = "https://app.crumbtrail.ai/sessions";
...
describe("buildAdvisoryComment", () => {
  ...
  it("renders an ambiguous comment with candidates and no claim about one session", () => {
    ...
    expect(rendered).toContain(
      "Candidate session: https://app.crumbtrail.ai/sessions/sessionAlpha (confidence 81%)",
    );
    expect(rendered).toContain(
      "Candidate session: https://app.crumbtrail.ai/sessions/sessionBeta (confidence 74%)",
    );
